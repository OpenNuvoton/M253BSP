 ...
 * @version  V1.00
 * @brief
 *           Demonstrate how to set I2C Master mode and Slave mode in 10-bit mode.
...
void I2C_SlaveTRx(uint32_t u32Status)
{
    ...
    {
        uint8_t u8Data = (uint8_t) I2C_GET_DATA(I2C1);
        s_au8SlvRxData[s_u8SlvDataLen] = u8Data;
        s_u8SlvDataLen++;

        if (s_u8SlvDataLen == 2)
        {
            uint32_t u32Data = (uint32_t)(s_au8SlvRxData[0] << 8);
            u32Data += s_au8SlvRxData[1];
            s_u32Slave_buff_addr = u32Data;
        }

        if (s_u8SlvDataLen == 3)
        {
            uint8_t u8Data = s_au8SlvRxData[2];
    ...
    {
        I2C_SET_DATA(I2C1, s_au8SlvData[s_u32Slave_buff_addr]);
...
void SYS_Init(void)
{
    ...
    SYS->GPB_MFPL = (SYS->GPB_MFPL & ~SYS_GPB_MFPL_PB2MFP_Msk)  |   SYS_GPB_MFPL_PB2MFP_I2C1_SDA;
    SYS->GPB_MFPL = (SYS->GPB_MFPL & ~SYS_GPB_MFPL_PB3MFP_Msk)  |   SYS_GPB_MFPL_PB3MFP_I2C1_SCL;
...
int32_t main(void)
{
    ...
    */
    ...
    printf("+-------------------------------------------------------+\n");
    printf("| I2C Driver Sample Code for loopback test (10bit mode) |\n");
    printf("|                                                       |\n");
