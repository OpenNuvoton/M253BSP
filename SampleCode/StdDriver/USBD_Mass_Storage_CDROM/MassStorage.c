...
void MSC_GetConfiguration(uint32_t u32Len, uint8_t *pu8Buf)
{
    ...
    if (g_u8Remove)
    {
        memset((int8_t *)pu8Buf, 0, u32Len);
        return;
    }

    if (g_sCBW.u8LUN == 0x02)
    {
        memcpy(pu8Buf, g_au8GetConfiguration, 8);
        u32FeatureLen = 0;
        ...
        while (u32Index < sizeof(g_au8GetConfiguration) - u32Index)
        {
            if ((g_au8GetConfiguration[u32Index] == g_sCBW.au8Data[0]) && (g_au8GetConfiguration[u32Index + 1] == g_sCBW.au8Data[1]))
            {
                ...
                memcpy(pu8Buf + 8, &g_au8GetConfiguration[u32Index], u32FeatureLen);
                break;
            }
            else
            {
    ...
    else if (g_sCBW.u8LUN == 0x01)
    {
        ...
        u32Len = pu8Ptr - pu8Buf;
        pu8Buf[3] = u32Len - 4;
    }
    else if (g_sCBW.u8LUN == 0x00)
        memcpy((char *)pu8Buf, &g_au8GetConfiguration[0], sizeof(g_au8GetConfiguration));
    else
    {
        g_au8SenseKey[0] = 0x05;
        g_au8SenseKey[1] = 0x24;
